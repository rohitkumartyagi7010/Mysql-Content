...
select
  distinct department,
  length(department) as DepartmentLength
  from worker;
...
-- only print length of department  

select length(department) as dep_length
from(select distinct department from worker) as uni;  
...
select distinct salary
       ...
       limit 1 offset 0;
-- also mention row number for each salary
select salary,
row_number() over(order by salary desc) as row_number1

from (select distinct salary from worker) as uni;       
...
select concat(upper(substring(first_name,1,1)),lower(substring(first_name,2))) as cap
from worker;       
